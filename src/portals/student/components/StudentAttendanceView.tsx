...
import { AttendanceRecord, AttendanceStatus, LeaveRequest, LeaveType, SectionCaptainInfo, Holiday } from '../../../types';
...
export const StudentAttendanceView: React.FC<StudentAttendanceViewProps> = ({
  ...
  useEffect(() => {
    ...
    const checkHoliday = async () => {
      ...
      try {
        const res = await api.getStudentActiveHoliday(targetDayIso, user?.batch, user?.section);
        if (isMounted) {
  ...
  }, [targetDayIso, user?.batch, user?.section]);
  ...
  const handleDirectStatusSwitch = async (newStatus: 'present' | 'absent', optionalReason?: string) => {
    ...
    if (activeHoliday) {
      setActionFeedback({
        type: 'error',
        message: `Attendance self-reporting is closed because '${activeHoliday.title}' (from ${activeHoliday.startDate} to ${activeHoliday.endDate}) is marked for your section by Class Captain.`,
      });
      return;
    }
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-2.5 pb-3 border-b border-slate-800/80">
            <div className="space-y-1">
              ...
              <h2 className="text-sm sm:text-base font-bold tracking-tight text-white flex items-center gap-1.5 flex-wrap">
                ...
                <span className={`${activeHoliday ? 'text-amber-300' : 'text-emerald-400'} font-mono`}>
                  {targetDayFormatted}
                </span>
              </h2>
              ...
              <div className="pt-2 border-t border-amber-400/20 flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 text-[11px] text-amber-200/90">
                <div className="flex items-center gap-1.5 min-w-0">
                  ...
                  <span className="truncate">
                    Marked by Class Captain: <strong className="text-white">{activeHoliday.createdBy?.name || 'Section Captain'}</strong>
